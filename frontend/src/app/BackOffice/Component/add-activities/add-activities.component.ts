...
export class AddActivitiesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.addAct = this.formBuilder.group({
      ...
      certification: [''],
      date_debut: ['', [Validators.required,dateValidator()]],
  ...
  addActivitie(){
this.activitiesService.addActivitie(this.addAct.value).subscribe(data=>{;
...
intitule:[''],
  ...
  removeFormateur(i:number){
this.formateur.removeAt(i);
  }
...
get DateStart(){
return this.addAct.get('date_debut');
}
isDateValid():boolean{
